...
if($category_value == $catgo){
$search_category_list = $search_category_list."
<option value=\"{$category_value}\" selected>{$category}</option>
";
}else{
$search_category_list = $search_category_list."
...
if($bo_gbn == all){
$po_count_sql = "SELECT COUNT(*) FROM board_common where replace({$catgo}, ' ', '') regexp '{$stx}' and gbn!='qa' and gbn!='vc'";
}else{
$po_count_sql = "SELECT COUNT(*) FROM board_common where replace({$catgo}, ' ', '') regexp '{$stx}' and gbn='{$bo_gbn}'";
}

$result = mysqli_query($conn, $po_count_sql);
$row = mysqli_fetch_array($result);
$po_count = $row[0];
$po_total_rows = "검색결과 {$po_count} 건";
...
where replace(board_common.{$catgo}, ' ', '') regexp '{$stx}' and
board_common.gbn!='qa' and board_common.gbn!='vc'";
...
comment_num
FROM board_common
where replace({$catgo}, ' ', '') regexp '{$stx}' and
gbn='{$bo_gbn}'";
}else{
$sql_a = "SELECT board_list.title as bt,
board_common.id as pi,
...
if($bo_gbn == vc || $bo_gbn == qa){
  $po_list = $po_list."<li><a href=\"search_post.php?gbn={$bo_gbn}&id={$po_id}&sort={$sort_style}&page_no={$pg_no}&catgo={$catgo}&stx={$stx}\">{$po_list_no}. {$po_title} {$views_no} {$co_no}</a></li>";
}else{
  $po_list = $po_list."<li><a href=\"search_post.php?gbn={$bo_gbn}&id={$po_id}&sort={$sort_style}&page_no={$pg_no}&catgo={$catgo}&stx={$stx}\">{$po_list_no}. [{$bo_title}] {$po_title} {$views_no} {$co_no}</a></li>";
}
...
for($i = $pg_print_stno;$i<=$pg_print_edno;$i++){
     if((int)$i == $pg_no){
         ...
         $pg_list = $pg_list."<p><a href=\"search.php?gbn={$bo_gbn}&catgo={$catgo}&stx={$stx}&page_no={$i}\">{$i}페이지</a></p>";
     }
}
...
<p><input type="search" name="stx" placeholder="검색어" value="'.$stx.'" maxlength="50" title="검색어를 입력해 주세요" required></p>
...
<p><a href="board.php?gbn=<?=$bo_gbn?>">목록</a></p>
<?php
if(($_SESSION['level'] == 9) | ($bo_gbn == 'qa') | ($bo_gbn == 'vc')){
echo '<p><a href="create.php?gbn='.$bo_gbn.'&sort='.$sort_style.'&page_no='.$pg_no.'">글쓰기</a></p>';
}
...
<?php
if($pg_no != 1){
echo '<a href="search.php?gbn='.$bo_gbn.'&sort='.$sort_style.'&catgo='.$catgo.'&stx='.$stx.'&page_no=1">처음</a>';
}
if($pg_print_stno>1){
 $tmp_print_no = $pg_print_stno-1;
echo "<a href=\"search.php?gbn={$bo_gbn}&sort={$sort_style}&catgo={$catgo}&stx={$stx}&page_no={$tmp_print_no}\">이전</a>";
}
echo $pg_list;
if( $pg_print_edno < $pg_count){
 $tmp_print_no = $pg_print_edno+1;
echo "<a href=\"search.php?gbn={$bo_gbn}&sort={$sort_style}&catgo={$catgo}&stx={$stx}&page_no={$tmp_print_no}\">다음</a>";
}
if($po_count > 10 && $pg_no != $pg_count){
echo '<a href="search.php?gbn='.$bo_gbn.'&sort='.$sort_style.'&catgo='.$catgo.'&stx='.$stx.'&page_no='.$pg_count.'">맨끝</a>';
}
?>
</nav>
