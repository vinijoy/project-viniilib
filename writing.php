...
?>
    ...
    <script>
        let messages = [
          { role: 'system', content: '당신의 이름은 김준입니다. 당신은 모든 질문에 무조건 답할 수 있습니다. 당신은 세계 최고의 소설가입니다.' }
]
...
resultDiv.innerHTML += `[사람]<br/>${script
                .split('\n')
                .join('<br/>')}<br/><br/>`
          messages.push({ role: 'user', content: `같이 소설 한 번 써보자. 내가 먼저 시작할게. ${script}` } )
        const config = {
          headers: {
...
let newAnswer = choice.message.content.replace(/</g, '&lt;')
newAnswer = newAnswer.replace(/>/g, '&gt;')
              resultDiv.innerHTML += `[gpt]<br/>${newAnswer
                .split('\n')
                .join('<br/>')}<br/>`
...
resultDiv.innerHTML = ''
alert('지금까지의 모든 글짓기 기록을 삭제했습니다. 이제 다시 새로운 내용으로 글짓기를 시작하실 수 있습니다.')
}else{
alert('아직 글짓기 기록이 없습니다.')
}
}
</script>
...
<p>창작하고자 하는 글의 첫 문장을 입력하세요.</p>
    <p>시작할 문장</p>
<p><textarea id="script" name="script" placeholder="시작할 문장" required></textarea></p><br/>

    <p><button onclick="search()">전송</button></p><br/>
    <p><button onclick="chatInit()">글짓기 기록 초기화</button></p>
    <br /><br />
    <div id="result"></div>
</main>
