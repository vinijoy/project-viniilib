...
views_num,
comment_num
FROM board_common where replace({$catgo}, ' ', '') regexp '{$stx}' and gbn!='qa' and gbn!='vc'";
}else{
$sql_a = "SELECT id,
title,
views_num,
comment_num
FROM board_common where replace({$catgo}, ' ', '') regexp '{$stx}' and gbn='{$bo_gbn}'";
}
$sql_b = "limit {$a}, {$b}";
$sql_sort = "";
...
for($i = $pg_print_stno;$i<=$pg_print_edno;$i++){
     if((int)$i == $pg_no){
         ...
         $pg_list = $pg_list."<p><a href=\"search.php?gbn={$bo_gbn}&sort={$sort_style}&catgo={$catgo}&stx={$stx}&page_no={$i}\">{$i}페이지</a></p>";
     }
}
...
$co_del_link = '';
if(($_SESSION['username'] == $co['username']) or ($_SESSION['level'] == 9)){
        $co_up_link = '<a href="update_comment.php?gbn='.$bo_gbn.'&id='.$filtered_id.'&cid='.$co['id'].'&sort='.$sort_style.'&page_no='.$pg_no.'&catgo='.$catgo.'&stx='.$stx.'">수정</a>';
        $co_del_link = '
